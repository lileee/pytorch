...
class SpatialConvolutionLocalBatch(Module):

    def __init__(self, nB, nInputPlane, nOutputPlane, iW, iH, kW, kH, dW=1, dH=1, padW=0, padH=None):
        super(SpatialConvolutionLocalBatch, self).__init__()
        ...
        self.weight = torch.Tensor(self.nB, self.oH, self.oW, nOutputPlane, nInputPlane, kH, kW)
        self.bias = torch.Tensor(self.nB, nOutputPlane, self.oH, self.oW)
    ...
    def _viewWeight(self):
        self.weight = self.weight.view(self.nB, self.oH * self.oW, self.nOutputPlane, self.nInputPlane * self.kH * self.kW)
        if self.gradWeight is not None and self.gradWeight.dim() > 0:
            self.gradWeight = self.gradWeight.view(
                self.nB, self.oH * self.oW, self.nOutputPlane, self.nInputPlane * self.kH * self.kW)
    ...
    def _checkInputSize(self, input):
        if input.ndimension() == 3:
            ...
            if input.size(0) != self.nB or input.size(1) != self.nInputPlane or input.size(2) != self.iH or input.size(3) != self.iW:
                raise RuntimeError(
                    'Given input size: ({}x{}x{}x{}) inconsistent with expected input size: ({}x{}x{}x{}).'.format(
                        input.size(0), input.size(1), input.size(2), input.size(3), self.nB, self.nInputPlane, self.iH, self.iW))
        else:
    ...
    def _checkOutputSize(self, input, output):
        ...
        if output.ndimension() == 3:
            ...
            if output.size(0) != self.nB or output.size(1) != self.nOutputPlane or output.size(2) != self.oH or output.size(3) != self.oW:
                raise RuntimeError('Given output size: ({}x{}x{}x{}) inconsistent with expected output size: '
                                   '({}x{}x{}x{}).'.format(
                                       output.size(0), output.size(1), output.size(2),
                                       output.size(3), self.nB, self.nOutputPlane, self.oH, self.oW))
        else:
            raise RuntimeError('3D or 4D(batch mode) tensor expected')
    ...
    def updateOutput(self, input):
        ...
        self._backend.SpatialConvolutionLocalBatch_updateOutput(
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self._backend.SpatialConvolutionLocalBatch_updateGradInput(
            self._backend.library_state,
            input,
            gradOutput,
            self.gradInput,
    ...
    def accGradParameters(self, input, gradOutput, scale=1):
        ...
        input, gradOutput = self._makeContiguous(input, gradOutput)
        self._viewWeight()
        self._backend.SpatialConvolutionLocalBatch_accGradParameters(
            self._backend.library_state,
            input,
            gradOutput,
            self.gradWeight,
    ...
    def type(self, type=None, tensorCache=None):
        ...
        return super(SpatialConvolutionLocalBatch, self).type(type, tensorCache)

    def __tostring__(self, ):
        s = super(SpatialConvolution, self).__repr__()
        s += '(B{}: {} -> {}, {}x{}, {}x{}'.format(self.nB, self.nInputPlane, self.nOutputPlane, self.iW, self.iH, self.kW, self.kH)
    ...
    def clearState(self):
        ...
        return super(SpatialConvolutionLocalBatch, self).clearState()
